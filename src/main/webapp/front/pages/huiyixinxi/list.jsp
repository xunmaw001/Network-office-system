		...
		<div id="app">
			...
			<div class="recommend index-pv1" :style='{"padding":"10px 0 10px 0","boxShadow":"0 0 6px rgba(255,254,255,1)","margin":"10px 0 0 0","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(255, 254, 255, 1)","borderRadius":"0","borderWidth":"0","background":"rgba(255,255,255,1)","borderStyle":"solid"}'>
				<div class="box" style='width:80%'>
					  ...
					  <span>DATA SHOW</span><span>会议信息展示</span>
					</div>
					...
					<form class="layui-form filter" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(110,22,64,.8)","margin":"10px 0 10px 0","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(39, 90, 187, 1)","borderRadius":"4px","borderWidth":"0","borderStyle":"solid","justifyContent":"flex-end","height":"64px"}'>
						...
						<div class="item-list">
							<div class="lable" :style='{"padding":"0 10px","boxShadow":"0 0 6px rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"transparent","color":"#fff","borderRadius":"0","textAlign":"right","borderWidth":"0","width":"auto","fontSize":"16px","borderStyle":"solid"}'>会议名称</div>
							<input type="text" :style='{"boxShadow":"0 0 6px rgba(255,0,0,0)","borderColor":"#ccc","backgroundColor":"#fff","color":"#333","borderRadius":"8px","textAlign":"center","borderWidth":"0","width":"140px","fontSize":"14px","borderStyle":"solid","height":"44px"}' name="huiyimingcheng" id="huiyimingcheng" placeholder="会议名称" autocomplete="off" class="layui-input">
						</div>
					<div class="item-list">
						<div class="lable" :style='{"padding":"0 10px","boxShadow":"0 0 6px rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"transparent","color":"#fff","borderRadius":"0","textAlign":"right","borderWidth":"0","width":"auto","fontSize":"16px","borderStyle":"solid"}'>会议部门</div>
						<select :style='{"boxShadow":"0 0 6px rgba(255,0,0,0)","borderColor":"#ccc","backgroundColor":"#fff","color":"#333","borderRadius":"8px","textAlign":"center","borderWidth":"0","width":"140px","fontSize":"14px","borderStyle":"solid","height":"44px"}' style="display:block" name="huiyibumen" id="huiyibumen" lay-filter="huiyibumen">
							<option value="">请选择</option>
							<option v-for="(item,index) in huiyibumenOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
						...
						<div class="list" style="position: relative;">
							<div @click="jump('../huiyixinxi/detail.jsp?id='+item.id)" v-for="(item,index) in dataList" :key="index" class="list-item" :class="3=='3'?'list-item3':3=='5'?'list-item5':''">
								<div class="list-item-body animation-box" :style='{"padding":"10px","boxShadow":"0 0 6px rgba(39,90,187,.8)","margin":"0 0 10px 0","borderColor":"rgba(0,0,0,.3)","backgroundColor":"#fff","borderRadius":"4px","borderWidth":"0","borderStyle":"solid"}'>
									<img :style='{"boxShadow":"0 0 6px rgba(39,90,187,.8)","borderColor":"rgba(0,0,0,.3)","borderRadius":"0","borderWidth":"0","width":"280px","borderStyle":"solid","height":"200px"}' :src="item.huiyitupian?baseurl+item.huiyitupian.split(',')[0]:''">
									<div class="info">
										...
										<div v-if="true" :style='{"isshow":true,"padding":"0","margin":"5px 0 0 0","backgroundColor":"rgba(0,0,0,0)","color":"rgba(29, 3, 3, 1)","borderRadius":"0","textAlign":"center","width":"100%","fontSize":"16px"}' class="name">{{item.huiyimingcheng}}</div>
									</div>
								</div>
							</div>
						</div>
			...
			layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate'], function() {
				...
				http.request('config/list', 'get', {
        			...
        			http.request(`option/bumen/bumen`,'get',{},(res)=>{
                			vue.huiyibumenOptions = res.data
        			});
				...
				function pageList() {
			        ...
			        if (jquery('#huiyimingcheng').val()) {
			          param['huiyimingcheng'] = jquery('#huiyimingcheng').val() ? '%' + jquery('#huiyimingcheng').val() + '%' : '';
			        }
			        if (jquery('#huiyibumen').val()) {
			          param['huiyibumen'] = jquery('#huiyibumen').val() ? jquery('#huiyibumen').val() : '';
			        }
				...
				if (jquery('#huiyibumen').val()) {
					...
					// 获取列表数据
					http.request('huiyixinxi/list', 'get', param, function(res) {
