...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑工作日程</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>工作日程管理</span></li>
								<li class="breadcrumb-item active"><span>编辑工作日程</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">工作日程信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>日程标题</label>
										<input name="richengbiaoti" id="richengbiaoti" name="richengbiaoti" class="form-control" placeholder="日程标题">
									</div>
									<div class="form-group col-md-6">
										<label>图片</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="yuangongxingming" id="yuangongxingming" name="yuangongxingming" class="form-control" placeholder="员工姓名">
											...
											<div class="form-group  col-md-12" >
												<label>日程内容</label>
												<div><div class="upload"><input id="richengneirongupload" name="file" type="file">选择文件</div>
												<script id="richengneirongEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var richengneirongUe = UE.getEditor('richengneirongEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="richengneirong" id="richengneirong-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "gongzuoricheng/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#richengneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('richengneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
				...
				if(crossrefid && crossuserid) {
					...
					http("gongzuoricheng/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("gongzuoricheng/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.richengneirong != null && ruleForm.richengneirong != 'null' && ruleForm.richengneirong != ''){
				var richengneirongMes = '' + ruleForm.richengneirong;
				var richengneirongUeditor = UE.getEditor('richengneirongEditor');
				richengneirongUeditor.ready(function() {
					richengneirongUeditor.setContent(richengneirongMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('richengneirongEditor').hasContents()){
				$('#richengneirong-input').attr('value',UE.getEditor('richengneirongEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("gongzuoricheng/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("gongzuoricheng"+colName,"GET",{
		...
		function dataBind(){
			...
			$("#richengbiaoti").val(ruleForm.richengbiaoti);	
			$("#richengneirong").val(ruleForm.richengneirong);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.richengbiaoti != null && res.data.richengbiaoti != ''){

						$("#richengbiaoti").val(res.data.richengbiaoti);
						$("#richengbiaoti").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.richengneirong != null && res.data.richengneirong != ''){

						$("#richengneirong").val(res.data.richengneirong);
                        var richengneirongMes = '' + res.data.richengneirong;
                        var richengneirongUeditor = UE.getEditor('richengneirongEditor');
                        richengneirongUeditor.ready(function() {
                            richengneirongUeditor.setContent(richengneirongMes);
                        });
